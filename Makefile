...
$(BUILD)/asaga_sparse.out: $(BUILD)/asaga_sparse.o $(OBJECTS) $(HEADERS)
	g++ $(BUILD)/asaga_sparse.o $(BUILD)/main.o  lib/libps.a -lprotobuf-lite -lpthread -lzmq -Llib -o $@ -std=c++11 -g -I${MKL_ROOT}/include -Wl,--start-group ${MKL_ROOT}/lib/intel64/libmkl_intel_lp64.a ${MKL_ROOT}/lib/intel64/libmkl_core.a ${MKL_ROOT}/lib/intel64/libmkl_sequential.a -Wl,--end-group
